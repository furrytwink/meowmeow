...
-- [ cosmetics feature — extracted to src/modules/cosmetics.lua; driver locals it uses are bound via ctx ]
...
end);guard("motiongraph", function()
	local MG = { On = false };
...
-- [ backtrack feature — extracted to src/modules/backtrack.lua; driver locals it uses are bound via ctx ]
guard("backtrack", function()
	return require("modules.backtrack")({
		ESP = ESP,
		Sections = Sections,
	});
end);guard("backtrack_chams", function()
...
-- [ world FX math helpers — extracted to src/modules/world_fx_math.lua; driver locals it uses are bound via ctx ]
guard("world_fx_math", function()
	return require("modules.world_fx_math")({
		ESP = ESP,
	});
end);guard("glyphs", function()
	local G = {
		On = false,
		Count = 55,
		Mode = "Theme",
...
-- [ weather effects feature — extracted to src/modules/weather.lua; driver locals it uses are bound via ctx ]
...
-- [ misc tweaks feature — extracted to src/modules/misc.lua; driver locals it uses are bound via ctx ]
guard("misc", function()
...
-- [ config save/load feature — extracted to src/modules/config.lua; driver locals it uses are bound via ctx ]
